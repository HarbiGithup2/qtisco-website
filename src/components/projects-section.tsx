...
export function ProjectsSection() {
  ...
  const projects = [
    {
      ...
      image: "/images/qtisco_image_5_2.jpeg", // يفضل أخذ ص��رة من PDF أو استبدالها لاحقًا
      services: ["Metallurgical Testing", "Non-Destructive Testing", "Third Party Inspection Services"],
      applications: [
    ...
    },
    {
      ...
      services: ["Civil Construction Material Testing", "Chemical Analysis"],
      applications: [
    ...
    },
    {
      ...
      services: ["Soil and Rock Testing", "Survey & Engineering Drafting Services"],
      applications: [
  ...
  return (
    <section className="py-5 bg-white relative overflow-hidden dark:bg-slate-900" id="projects-section">
      {/* Background Pattern */}
      ...
      <div className="container mx-auto px-4 relative">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl lg:text-5xl font-bold text-slate-900 mb-6 animate-slideInUp dark:text-white">
            Comprehensive Testing <span className="text-red-600 dark:text-red-400">Solutions</span>
          </h2>
          <p
            className="text-xl text-slate-600 max-w-3xl mx-auto leading-relaxed animate-fadeInUp dark:text-slate-300"
            style={{ animationDelay: "0.2s" }}
          >
            With over a decade of experience in the field of testing & laboratory services, QTISCO has executed a number
            of projects in Oil, Power, Petro-chemical sectors of Kuwait. QTISCO is embarking its journey to the kingdom
            with all its experiences and resources.
          </p>
        </div>
            ...
            <Button
              ...
              className={`px-6 py-2 transition-all duration-300 ${
                activeFilter === category
                  ...
                  : "border-slate-300 text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-700"
              }`}
            >
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          {filteredProjects.map((project, index) => (
            <Card
              key={index}
              className="group cursor-pointer border-0 shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-3 bg-white overflow-hidden animate-floatUp dark:bg-slate-900 dark:border dark:border-slate-700"
            ...
            >
              <Link href={project.link} aria-label={`Learn more about ${project.title}`}>
                ...
                <CardContent className="p-6">
                  {/* Project Header */}
                  <div className="mb-4">
                    <h3 className="text-xl font-bold text-slate-900 mb-2 group-hover:text-red-600 transition-colors duration-300 dark:text-white dark:group-hover:text-red-400">
                      {project.title}
                    </h3>
                    <p className="text-slate-600 leading-relaxed text-sm dark:text-slate-300">{project.description}</p>
                  </div>

                  {/* Services Used */}
                  <div className="mb-4">
                    <h4 className="font-semibold text-slate-900 mb-2 text-sm dark:text-white">Services:</h4>
                    <div className="flex flex-wrap gap-1">
                      {project.services.map((service, serviceIndex) => (
                        <span
                          key={serviceIndex}
                          className="px-2 py-1 bg-blue-50 text-blue-700 text-xs rounded-md font-medium dark:bg-blue-900/20 dark:text-blue-400"
                        >
                          {service}
                        </span>
                      ))}
                  ...
                  {/* Applications */}
                  <div className="mb-4">
                    <h4 className="font-semibold text-slate-900 mb-2 text-sm dark:text-white">Applications:</h4>
                    <div className="space-y-1">
                      {project.applications.slice(0, 3).map((application, appIndex) => (
                        ...
                        >
                          <CheckCircle className="h-3 w-3 text-green-500 mt-0.5 flex-shrink-0 dark:text-green-400" />
